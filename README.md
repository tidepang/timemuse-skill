...
安装后只需确认一次：允许 AI 按需读取时间块、备注、想法、已保存日反馈、Todo 和周目标的历史记录。相关内容会交给当前 AI 服务处理；Skill 只读，不修改原始记录，也不读取截图或 OCR。Agent 安装时，先转述这项确认，得到同意后运行已安装目录中的 `python3 scripts/evidence.py setup --yes`。安装本身不会启用读取。
